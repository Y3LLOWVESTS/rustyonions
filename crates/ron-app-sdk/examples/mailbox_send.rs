...
async fn main() -> Result<()> {
    ...
    if let Some(k) = idem { payload["idempotency_key"] = serde_json::Value::String(k); }
    ...
    let resp = framed.next().await.ok_or_else(|| anyhow!("no response"))??;
    if !resp.flags.contains(OapFlags::RESP) { return Err(anyhow!("expected RESP")); }
    if resp.code != 0 { return Err(anyhow!(String::from_utf8_lossy(&resp.payload).to_string())); }
    ...
    -> Result<tokio_rustls::client::TlsStream<TcpStream>>
{
    ...
    let sockaddr = addr.to_socket_addrs()?.next().ok_or_else(|| anyhow!("addr resolve failed"))?;
    let tcp = TcpStream::connect(sockaddr).await?;
    tcp.set_nodelay(true)?;

    let mut roots = RootCertStore::empty();
    for cert in rustls_native_certs::load_native_certs().map_err(|e| anyhow!("native certs: {e}"))? {
    ...
    let sni: ServerName = ServerName::try_from(server_name.to_string()).map_err(|_| anyhow!("invalid sni"))?;
