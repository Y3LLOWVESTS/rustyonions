<!-- Generated by scripts/make_crate_codex.sh on 2026-07-13T05:26:46Z -->
...
- [crates/ron-policy/tests/fixtures/roc-paid-action-economics.legacy.toml](#crates-ron-policy-tests-fixtures-roc-paid-action-economics-legacy-toml)
...
//! RO:INVARIANTS — legacy values preserved; nested validation enforced; hash order stable.
//! RO:METRICS — none.
...
use ron_policy::economics::{
    canonical_internal_roc_economics_bytes, internal_roc_economics_config_hash,
    load_economics_toml_str, load_internal_roc_economics_toml,
    validate_internal_roc_economics_config, InternalRocEconomicsConfig,
};

const CANONICAL: &str = include_str!("../../../configs/roc-economics.toml");

const LEGACY_PAID_ACTIONS: &str = include_str!("fixtures/roc-paid-action-economics.legacy.toml");
...
#[test]
fn canonical_config_preserves_existing_paid_action_policy() {
    let unified = canonical_config();

    let legacy = load_economics_toml_str(LEGACY_PAID_ACTIONS)
        .expect("legacy paid-action fixture must validate");

    assert_eq!(unified.paid_actions, legacy);
...
fn load_checked_in_raw(raw: &str) -> EconomicsPolicy {
    load_internal_roc_economics_toml(raw.as_bytes())
        .expect("checked-in economics config should load")
        .paid_actions
}

fn load_checked_in() -> EconomicsPolicy {
    load_checked_in_raw(CHECKED_IN_POLICY)
}
...
<a id="crates-ron-policy-tests-fixtures-roc-paid-action-economics-legacy-toml"></a>
