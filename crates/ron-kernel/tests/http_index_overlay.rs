...
use ron_kernel::{Metrics, wait_for_ctrl_c};
...
#[derive(Deserialize)]
struct PutReq { addr: String, dir: String }

#[derive(Serialize)]
struct EchoResp { echo: String }

async fn overlay_echo(axum::extract::State(st): axum::extract::State<TestState>, Json(req): Json<serde_json::Value>) -> impl axum::response::IntoResponse {
    let _t = st.metrics.request_latency_seconds.start_timer();
    sleep(Duration::from_millis(2)).await;
    let payload = req.get("payload").and_then(|v| v.as_str()).unwrap_or_default().to_string();
    (StatusCode::OK, Json(EchoResp { echo: payload }))
}

async fn index_put(axum::extract::State(st): axum::extract::State<TestState>, Json(req): Json<PutReq>) -> impl axum::response::IntoResponse {
    let _t = st.metrics.request_latency_seconds.start_timer();
    st.map.write().await.insert(req.addr, req.dir);
    (StatusCode::OK, Json(serde_json::json!({"ok": true, "data": "ok"})))
}

async fn index_resolve(axum::extract::State(st): axum::extract::State<TestState>, axum::extract::Path(addr): axum::extract::Path<String>) -> impl axum::response::IntoResponse {
    let _t = st.metrics.request_latency_seconds.start_timer();
    let g = st.map.read().await;
    if let Some(dir) = g.get(&addr) {
        (StatusCode::OK, Json(serde_json::json!({"ok": true, "data": { "addr": addr, "dir": dir }})))
    } else {
...
async fn overlay_echo_roundtrip() {
    ...
    let st = TestState { metrics: m.clone(), map: std::sync::Arc::new(tokio::sync::RwLock::new(Default::default())) };
    ...
    let r = client.post(format!("http://{addr}/echo"))
...
async fn index_put_resolve_roundtrip() {
    ...
    let client = reqwest::Client::new();
    for i in 1..=3 {
        let r = client.post(format!("http://{addr}/put"))
