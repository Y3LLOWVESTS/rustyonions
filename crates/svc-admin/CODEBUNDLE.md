<!-- Generated by scripts/make_crate_codex.sh on 2025-12-12T22:42:05Z -->
...
///
/// Read-only, curated storage facts for the node (disk usage, I/O rates, mounts).
///
/// Invariant: this is NOT a remote file browser. Node admin-planes should expose
/// a whitelisted DTO under something like `/api/v1/storage/summary`.
///
/// Slice 3d: route scaffold only.
/// - Returns 404 if node id not registered.
/// - Returns 501 until node/admin-plane + svc-admin wiring exists.
...
///
/// Read-only database inventory for the node (logical db name, engine, size, perms, health).
///
/// Slice 3d: route scaffold only.
...
///
/// Read-only database detail for a single database entry.
///
/// Slice 3d: route scaffold only.
...
///
/// Dev-only hook to proxy synthetic crash events to the node admin plane.
/// This is primarily used to exercise restart counters and observability
/// flows from the UI without killing real workers.
///
/// Gates:
///   - We currently rely on the node itself to enforce `MACRONODE_DEV_INSECURE`
///     or admin auth on `/api/v1/debug/crash`.
///   - svc-admin still requires the node id to exist in the registry.
...
function buildUrl(path: string): string {
  if (!RAW_BASE_URL) {
    // Default: same-origin / Vite proxy path.
    return path
  }
...
async function handleResponse<T>(res: Response): Promise<T> {
  ...
  if (!text) {
    ...
    // In those cases, the caller should declare T as void or similar.
...
export const adminClient = {
  ...
  //
  // These endpoints may not exist yet on the svc-admin backend. Callers
  // should be prepared to handle 404/501 and fall back to mock data while
  // node/admin-plane support rolls out.
  ...
  async debugCrashNode(
    ...
    // Backend expects JSON via `Json<DebugCrashRequest>`:
    //   { "service": "svc-gateway" } or {} when omitted.
...
//
// WHAT:
//   Right-hand sidebar on Node Detail page (deep-dive companion panel).
//
// WHY:
//   Gives operators fast “secondary” intel + shortcuts (data-plane summary,
//   metrics freshness, and now a dedicated Data & storage entrypoint).
//
// INTERACTS:
//   - NodeDetailPage (passes status + metricsHealth)
//   - NodeStoragePage (via /nodes/:id/storage route)

import React from 'react'
import { Link } from 'react-router-dom'

import type { AdminStatusView } from '../../types/admin-api'
import type { MetricsFreshness } from '../../types/metrics'

type DataPlaneHealth = 'healthy' | 'degraded' | 'down' | 'unknown'
...
function toBool(v: unknown): boolean {
  ...
  if (typeof v === 'string') return v.toLowerCase() === 'ready' || v === 'true'
  return false
}
...
function normalizeHealth(v: unknown): DataPlaneHealth {
...
function extractDataPlanes(status: AdminStatusView | null): DataPlaneRow[] {
  if (!status) return []

  const planesRaw = (status as any).planes
  if (!Array.isArray(planesRaw)) return []

  // Convention: planes table contains both core/admin and data-plane services.
  // For sidebar we focus on data-plane services (svc-*) where possible.
  const rows: DataPlaneRow[] = planesRaw
    .map((p: any) => {
      const name: string = p?.name ?? p?.plane ?? p?.service ?? 'unknown'
      const ready: boolean = toBool(p?.ready ?? p?.is_ready ?? p?.isReady)
      const restarts: number = toInt(
        p?.restarts ?? p?.restart_count ?? p?.restartCount,
      )
      const health: DataPlaneHealth = normalizeHealth(
        p?.health ?? p?.status ?? p?.state,
      )

      // Label polish (show svc-* as-is, otherwise friendly)
      const label =
        typeof name === 'string' && name.length > 0 ? name : 'unknown'

      return { name, label, ready, restarts, health }
    })
    // Heuristic: data-plane is the svc-* set
    .filter((r) => typeof r.name === 'string' && r.name.startsWith('svc-'))

  return rows
}

function DataPlaneHealthChip({ health }: { health: DataPlaneHealth }) {
  const cls = [
    'svc-admin-node-detail-chip',
    `svc-admin-node-detail-chip--${health}`,
  ].join(' ')
  return <span className={cls}>{health}</span>
}

function renderMetricsBanner(metricsHealth: MetricsFreshness | null) {
  if (!metricsHealth) return null
...
function hasStorageCapability(status: AdminStatusView | null): boolean {
  ...
  if (!Array.isArray(caps)) return true // if not reported yet, don’t block in dev
...
export function NodeDetailSidebar({
  ...
  metricsHealth: MetricsFreshness | null
  loading?: boolean
}) {
  const dataPlanes = extractDataPlanes(status)
  const metricsBanner = renderMetricsBanner(metricsHealth)
  ...
  return (
    <aside className="svc-admin-node-detail-sidebar">
      {/* Data & storage ----------------------------------------------------- */}
      <section className="svc-admin-node-detail-sidebar-card">
        <h2 className="svc-admin-node-detail-sidebar-title">Data &amp; storage</h2>
        <p className="svc-admin-node-detail-sidebar-caption">
          Databases, disk usage, and safe storage facts (read-only).
        </p>

        {canOpenStorage ? (
          <Link to={storageHref} className="svc-admin-node-preview-open">
            Open storage &amp; databases <span className="svc-admin-node-preview-open-icon">→</span>
          </Link>
        ) : (
      ...
      {/* Data-plane summary ------------------------------------------------- */}
      <section className="svc-admin-node-detail-sidebar-card">
        <h2 className="svc-admin-node-detail-sidebar-title">Data planes</h2>
        <p className="svc-admin-node-detail-sidebar-caption">
          Quick read on svc-* planes (health, readiness, restarts).
        </p>
        ...
        {!loading && (!dataPlanes || dataPlanes.length === 0) && (
          <div className="svc-admin-node-detail-sidebar-empty">
            No explicit data-plane services reported for this node yet.
          </div>
        )}
        ...
        {metricsBanner && (
          <div className="svc-admin-node-detail-sidebar-metrics-wrap">
            {metricsBanner}
          </div>
        )}
      </section>
      ...
      {/* Playground stub --------------------------------------------------- */}
      <section className="svc-admin-node-detail-sidebar-card">
        <h2 className="svc-admin-node-detail-sidebar-title">Playground</h2>
        <p className="svc-admin-node-detail-sidebar-caption">
          Future home for a read-only playground (safe queries, targeted
          metrics, and structured logs) scoped to this node. For now this is
          just a stub.
        </p>

        <div className="svc-admin-node-detail-playground">
          <textarea
            className="svc-admin-node-detail-playground-input"
            rows={4}
            disabled
          ...
          <button
            type="button"
            className="svc-admin-node-detail-playground-button"
            disabled
          >
            Run (coming soon)
          </button>
...
//
// NOTE:
//   - This panel now supports an optional "Planes" table, matching the mock.
...
type Props = {
  ...
  // NEW: optional per-plane table (pass selected node's planes array if available)
                  ...
                  return (
                    <tr key={String(name)}>
                      ...
                      <td>
                        ...
                        >
                          {h === 'unknown' ? 'Unknown' : h.charAt(0).toUpperCase() + h.slice(1)}
                        </span>
                      </td>
                      <td>
                        <span
...
// crates/svc-admin/ui/src/components/nodes/PlaneStatusTable.tsx
//
// RO:WHAT — Table view for a node's planes (gateway/storage/index/...).
// RO:WHY  — Gives operators a compact, glanceable view of health/ready/restarts
//           per plane without diving into logs or raw JSON.
...
// Tiny helper to render a restart count as a pill with severity-based color.
// 0   → neutral gray.
// 1-2 → soft amber ("watch this").
// 3+  → red ("this plane is flapping").
function RestartPill({ count }: { count: number }) {
  const base =
    'inline-flex items-center justify-end rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide border'

  if (count === 0) {
    return (
      <span
        className={`${base} border-slate-700 bg-slate-900/60 text-slate-400`}
      >
        0
      </span>
    )
  ...
  if (count <= 2) {
    return (
      <span
        className={`${base} border-amber-400/70 bg-amber-500/10 text-amber-200`}
      >
        {count}
      </span>
    )
  ...
  return (
    <span
      className={`${base} border-rose-500/80 bg-rose-500/10 text-rose-200`}
    >
      {count}
    </span>
  )
...
export function PlaneStatusTable({ planes }: Props) {
  if (!planes.length) {
    return (
      <div className="rounded-lg border border-slate-800/80 bg-slate-950/40 px-4 py-3 text-xs text-slate-400">
        No plane information reported yet for this node.
      </div>
    )
  }
      ...
      <table className="min-w-full border-separate border-spacing-0 text-xs">
        <thead>
          <tr className="bg-slate-950/70">
            <th className="border-b border-slate-800 px-4 py-2 text-left font-semibold text-slate-200">
              Plane
            </th>
            <th className="border-b border-slate-800 px-4 py-2 text-left font-semibold text-slate-200">
              Health
            ...
            <th className="border-b border-slate-800 px-4 py-2 text-right font-semibold text-slate-200">
              Restarts
            </th>
          </tr>
        </thead>
        <tbody>
          {planes.map((plane) => (
            <tr key={plane.name} className="odd:bg-slate-950/40">
              ...
              <td className="px-4 py-2">
                <span
                  className={
                    plane.health === 'healthy'
                      ? 'inline-flex rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300'
                      : plane.health === 'degraded'
                        ? 'inline-flex rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold text-amber-300'
                        : 'inline-flex rounded-full bg-rose-500/10 px-2 py-0.5 text-[10px] font-semibold text-rose-300'
                  }
                >
                  {plane.health}
                </span>
              ...
              <td className="px-4 py-2 pr-4 text-right align-middle">
                {/* PlaneStatus from admin-api.ts uses `restart_count` (snake_case). */}
                <RestartPill count={plane.restart_count} />
              </td>
            </tr>
          ))}
        </tbody>
...
//           to deterministic mock data when endpoints are missing (404/501).
// RO:INTERACTS —
...
function mockStorageSummary(nodeId: string): StorageSummaryDto {
  // Deterministic-ish mock values so refreshes look stable per node.
...
function modeLooksWorldReadable(mode: string): boolean {
  // Very rough: "....5" or "....4" or "....6" or "....7" means "other" has read.
...
export function NodeStoragePage() {
  ...
  // --- storage/db endpoints (new, optional) -------------------------------
  ...
  // Load storage summary (optional endpoint)
  useEffect(() => {
    if (!nodeId) return
    let cancelled = false
    ...
    ;(async () => {
      try {
        ...
        if (isMissingEndpoint(err)) {
          ...
          setStorageError(msg)
          // Still show mock so the page is usable.
          setStorage(mockStorageSummary(nodeId))
          setStorageSource('mock')
        }
      } finally {
  ...
  // Load database inventory (optional endpoint)
  useEffect(() => {
    if (!nodeId) return
    let cancelled = false
  ...
  // Load database detail on selection (optional endpoint)
  useEffect(() => {
    if (!nodeId) return
    if (!selectedDb) {
      setDbDetail(null)
    ...
    ;(async () => {
      try {
        ...
        if (isMissingEndpoint(err)) {
          ...
          const msg =
            err instanceof Error ? err.message : 'Failed to load database detail.'
          setDbDetailError(msg)
          // Still show mock so operator can keep moving.
          setDbDetail(mockDatabaseDetail(nodeId, selectedDb))
        }
      } finally {
  ...
  const usedPct = s.totalBytes ? Math.round((s.usedBytes / s.totalBytes) * 100) : 0
  const freePct = 100 - usedPct
  const lowDisk = freePct <= 10

  const sourceLabel =
    storageSource === 'live' && dbSource === 'live'
      ? 'Live'
  ...
  return (
    <div className="svc-admin-page svc-admin-page-node-storage">
      ...
      {!facetsLoading && facetsError && (
        <ErrorBanner message="Metrics unreachable for this node." />
      )}

      {(storageError || dbError) && (
        <ErrorBanner
          message="Storage endpoints not fully available yet. Showing mock data where needed."
        />
      )}

      {lowDisk && (
        <ErrorBanner
          message={`Low disk headroom: ~${freePct}% free on ${s.mount}.`}
        />
      )}
      ...
      <section className="svc-admin-section" style={{ marginBottom: '1rem' }}>
        ...
        {storageLoading ? (
          ...
          >
            ...
            <div className="svc-admin-card" style={{ padding: '0.9rem 1rem' }}>
              ...
              <div style={{ fontSize: '0.9rem', opacity: 0.8 }}>{freePct}% headroom</div>
            </div>
        ...
        <p style={{ marginTop: '0.75rem', fontSize: '0.9rem', opacity: 0.85 }}>
          Storage/DB facts are curated and read-only. When node support is not present,
          this page safely falls back to mock data.
        </p>
      </section>
      ...
      <section className="svc-admin-section">
        ...
        {dbLoading ? (
          ...
          >
            <div className="svc-admin-card" style={{ padding: 0, overflow: 'hidden' }}>
              <table className="svc-admin-plane-table" style={{ marginTop: 0 }}>
                ...
                <tbody>
                  {databases.map((db) => {
                    ...
                    if (db.notes) titleBits.push(db.notes)
                    if (db.worldWritable) titleBits.push('⚠ world-writable')
                    else if (db.worldReadable) titleBits.push('⚠ world-readable')
            ...
            <div className="svc-admin-card" style={{ padding: '1rem' }}>
              {!selectedDb ? (
                <div style={{ opacity: 0.85 }}>Select a database to inspect details.</div>
              ) : dbDetailLoading ? (
                <LoadingSpinner />
              ) : dbDetailError ? (
                <ErrorBanner message="Failed to load database detail; showing mock fallback." />
              ) : !dbDetail ? (
                <div style={{ opacity: 0.85 }}>No detail available.</div>
              ) : (
                <div>
                  <div
                    style={{
                  ...
                  >
                    <h3 style={{ margin: 0 }}>{dbDetail.name}</h3>
                  ...
                  >
                    <dt style={{ fontWeight: 650 }}>Size</dt>
                    <dd style={{ margin: 0 }}>{fmtBytes(dbDetail.sizeBytes)}</dd>

                    <dt style={{ fontWeight: 650 }}>Owner</dt>
                    <dd style={{ margin: 0 }}>{dbDetail.owner}</dd>

                    <dt style={{ fontWeight: 650 }}>Mode</dt>
                    <dd style={{ margin: 0 }}>{dbDetail.mode}</dd>

                    <dt style={{ fontWeight: 650 }}>Path alias</dt>
                    <dd style={{ margin: 0 }}>{dbDetail.pathAlias}</dd>

                    <dt style={{ fontWeight: 650 }}>Files</dt>
                    <dd style={{ margin: 0 }}>{dbDetail.fileCount.toLocaleString()}</dd>

                    <dt style={{ fontWeight: 650 }}>Keys</dt>
                    <dd style={{ margin: 0 }}>
                      {dbDetail.approxKeys === null ? 'n/a' : dbDetail.approxKeys.toLocaleString()}
                    </dd>

                    <dt style={{ fontWeight: 650 }}>Compaction</dt>
                    <dd style={{ margin: 0 }}>{dbDetail.lastCompaction ?? 'n/a'}</dd>

                    <dt style={{ fontWeight: 650 }}>Health</dt>
                    <dd style={{ margin: 0, textTransform: 'capitalize' }}>
                      {dbDetail.health}
                    </dd>
                  </dl>

                  {dbDetail.warnings.length > 0 && (
                    <div style={{ marginTop: '0.9rem' }}>
                      <div style={{ fontWeight: 700, marginBottom: '0.35rem' }}>
                        Warnings
                      </div>
                      <ul style={{ margin: 0, paddingLeft: '1.1rem' }}>
                        {dbDetail.warnings.map((w) => (
...
// - `last_sample_age_secs` is seconds since the last sample, or null
//   when the sampler has never seen this facet for this node.
...
//
// These DTOs will be mirrored on the Rust side once svc-admin backend routes
// exist for /api/nodes/:id/storage/* and node admin-planes expose
// /api/v1/storage/*.
...
export type StorageSummaryDto = {
  ...
  // Optional extra fields for future expansion (mount list, inode headroom, etc.)
  // Keep this stable and additive.
}
