<!-- Generated by scripts/make_crate_codex.sh on 2025-11-21T18:58:41Z -->
...
///
/// Today this is "fire-and-forget": each service is a stub worker that runs
/// until the shared `ShutdownToken` is triggered. Future slices will return
/// join handles and wire crash detection / restart policies.
pub async fn spawn_all(probes: Arc<ReadyProbes>, shutdown: ShutdownToken) -> Result<()> {
    ...
    // Now takes probes so it can mark gateway_bound = true when the listener
    // successfully binds.
    crate::services::svc_gateway::spawn(probes.clone());

    // Stub workers for the remaining services; they just loop until shutdown.
    ...
    // All stub deps are considered "ok" once their workers have been spawned.
