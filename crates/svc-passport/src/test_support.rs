...
// RO:WHY   Integration tests compile as separate crates, so they import through
//          the external crate name `svc_passport::…`.
...
fn load_cfg_for_tests() -> Config {
    ...
    // Otherwise, inject the embedded TOML through PASSPORT_CONFIG so the crate’s
    // own loader will parse it without extra dev-deps in the test crate.
