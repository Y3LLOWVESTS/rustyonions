...
use svc_passport::{
    ...
    profile::{UsernameClaimRequest, UsernameClaimStatus, UsernameClaimStore},
};
...
const SUBJECT: &str = "passport:main:forum_creator";

const USERNAME: &str = "forum_creator";
...
fn claim_request() -> UsernameClaimRequest {
    UsernameClaimRequest {
        ...
        display_name: Some("Forum Creator".to_owned()),

        bio: Some("Forum publication identity.".to_owned()),
...
fn phase15a4a2a_core_reverse_lookup_returns_confirmed_claimed_profile() {
    ...
    store
        .claim_main_username(claim_request(), 1_776_000_000_000)
        .expect("claim succeeds");

    let profile = store
        .public_profile_for_passport_subject(SUBJECT)
        .expect("subject lookup succeeds")
        .expect("profile exists");

    assert_eq!(profile.passport_subject, SUBJECT,);

    assert_eq!(profile.username, USERNAME,);

    assert_eq!(profile.profile_crab_url, "crab://@forum_creator",);
...
fn phase15a4a2a_unknown_passport_subject_returns_no_profile_without_guessing_username() {
    ...
    let profile = store
        .public_profile_for_passport_subject("passport:main:unknown_creator")
        .expect("unknown lookup is valid");
...
async fn phase15a4a2a_http_claim_then_subject_lookup_returns_same_confirmed_identity() {
    ...
    let claim_response = app.clone().oneshot(claim).await.expect("claim response");

    assert_eq!(claim_response.status(), http::StatusCode::CREATED,);

    let lookup = Request::builder()
        .method(http::Method::GET)
        .uri("/v1/passport/profile/by-subject/passport%3Amain%3Aforum_creator")
        .body(Body::empty())
        .expect("lookup request");

    let lookup_response = app.clone().oneshot(lookup).await.expect("lookup response");

    assert_eq!(lookup_response.status(), http::StatusCode::OK,);

    let body = response_json(lookup_response).await;

    assert_eq!(body["passport_subject"], SUBJECT,);

    assert_eq!(body["username"], USERNAME,);

    assert_eq!(body["profile_crab_url"], "crab://@forum_creator",);
...
async fn phase15a4a2a_http_unknown_subject_returns_not_found() {
    ...
    let request = Request::builder()
        .method(http::Method::GET)
        .uri("/v1/passport/profile/by-subject/passport%3Amain%3Amissing_creator")
        .body(Body::empty())
        .expect("lookup request");

    let response = app.oneshot(request).await.expect("lookup response");
...
fn phase15a4a2a_source_boundary_keeps_subject_lookup_read_only_and_store_derived() {
    let core_source = include_str!("../src/profile.rs",);

    let handler_source = include_str!("../src/http/handlers/profile.rs",);

    let router_source = include_str!("../src/http/router.rs",);
    ...
    for required in [
        ...
        assert!(
            handler_source.contains(required,) || router_source.contains(required,),
