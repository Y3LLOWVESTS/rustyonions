...
async fn start_dummy_omnigate() -> SocketAddr {
    ...
    async fn relation_handler(uri: Uri, headers: HeaderMap) -> (StatusCode, Json<Value>) {
    ...
    let router = Router::new()
        ...
        .route("/v1/publication-relations", get(relation_handler));
...
async fn phase14a6d_gateway_preserves_relation_query_and_headers() {
    ...
    let response = reqwest::Client::new()
        .get(format!("http://{gateway_address}/publication-relations",))
        .query(&[
            ("parentCrabUrl", IMAGE_URL),
            ("cursor", "r_00000002"),
            ("limit", "30"),
        ])
        .header("authorization", "Bearer gateway-relation-read")
        .header("x-correlation-id", "corr-gateway-phase14a6d")
        .header("x-request-id", "req-gateway-phase14a6d")
        .header("connection", "close")
        .send()
        .await
        .expect("gateway relation response");

    assert_eq!(response.status(), StatusCode::OK,);

    let body: Value = response.json().await.expect("gateway relation JSON");

    assert_eq!(body["schema"], "crablink.publication-relation-page.v1",);

    assert_eq!(body["path"], "/v1/publication-relations",);

    assert_eq!(body["parentCrabUrl"], IMAGE_URL,);

    assert_eq!(body["cursor"], "r_00000002",);

    assert_eq!(body["limit"], "30",);

    assert_eq!(body["authorization"], "Bearer gateway-relation-read",);

    assert_eq!(body["xCorrelationId"], "corr-gateway-phase14a6d",);

    assert_eq!(body["xRequestId"], "req-gateway-phase14a6d",);
...
async fn phase14a6d_gateway_transport_failure_is_structured_bad_gateway() {
    ...
    let response = reqwest::Client::new()
        ...
        .send()
        .await
        .expect("gateway relation transport response");
    ...
    let body: Value = response
        .json()
        .await
        .expect("gateway relation bad gateway JSON");
