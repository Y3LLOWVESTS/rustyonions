...
fn test_metrics_handles(
    ...
    static CELL:
        OnceCell<metrics::MetricsHandles> =
            OnceCell::new();
...
async fn phase6b4_list_route_preserves_query_and_headers() {
    ...
    assert_eq!(
        body["path"],
        "/v1/creators/rusty_crab/publications",
    );
...
async fn phase6b4_upstream_query_rejection_passes_through() {
    ...
    let response =
        reqwest::Client::new()
            ...
            .await
            .expect(
                "gateway rejected query response",
            );
