...
async fn dummy_explore(
    ...
    if query.get("mode").map(String::as_str) == Some("problem400") {
        return (
...
async fn start_dummy_omnigate() -> SocketAddr {
    let router = Router::new().route("/v1/explore", get(dummy_explore));
...
async fn start_gateway(omnigate_address: SocketAddr) -> SocketAddr {
    std::env::set_var(
        ...
        ["http://", &omnigate_address.to_string()].concat(),
    );

    std::env::set_var("SVC_GATEWAY_BIND_ADDR", "127.0.0.1:0");

    let config = Config::load().expect("load gateway config");
    ...
    wait_for_health(["http://", &address.to_string(), "/healthz"].concat()).await;

    address
}

#[tokio::test]
async fn phase10a3d_public_explore_targets_omnigate() -> TestResult {
    ...
    let response = reqwest::Client::new()
        .get(["http://", &gateway_address.to_string(), "/explore"].concat())
        .send()
        .await?;
    ...
    let body: Value = response.json().await?;
        ...
        body.pointer("/observed/path").and_then(Value::as_str) == Some("/v1/explore"),
...
#[tokio::test]
async fn phase10a3d_explore_query_is_preserved_for_omnigate_validation() -> TestResult {
    let _guard = ENV_LOCK.lock().await;
    ...
    let body: Value = response.json().await?;
        ...
        body.pointer("/observed/query/publicationLimit")
        ...
        body.pointer("/observed/query/creatorLimit")
            .and_then(Value::as_str)
            == Some("20"),
        ...
        body.pointer("/observed/query/siteLimit")
            .and_then(Value::as_str)
            == Some("16"),
...
#[tokio::test]
async fn phase10a3d_selected_headers_forward_and_hop_by_hop_header_does_not() -> TestResult {
    let _guard = ENV_LOCK.lock().await;
    ...
    let response = reqwest::Client::new()
        ...
        .header("x-correlation-id", "phase10a3d-correlation")
    ...
    let body: Value = response.json().await?;
        ...
        body.pointer("/observed/authorization")
            .and_then(Value::as_str)
            == Some("Bearer dev"),
        ...
        body.pointer("/observed/correlationId")
            .and_then(Value::as_str)
            .is_some(),
        ...
        body.pointer("/observed/connection").map(Value::is_null) == Some(true),
...
#[tokio::test]
async fn phase10a3d_omnigate_error_status_and_body_pass_through() -> TestResult {
    let _guard = ENV_LOCK.lock().await;
    ...
    let body: Value = response.json().await?;
        ...
        body.get("code").and_then(Value::as_str) == Some("invalid_explore_query"),
...
#[tokio::test]
async fn phase10a3d_omnigate_transport_failure_is_structured_gateway_502() -> TestResult {
    let _guard = ENV_LOCK.lock().await;
    ...
    let config = Config::load().expect("load unavailable Omnigate config");

    let state = AppState::new(config.clone(), test_metrics_handles());

    let listener = TcpListener::bind(&config.server.bind_addr)
        .await
        .expect("bind failure-path gateway");

    let gateway_address = listener.local_addr().expect("failure-path gateway address");

    tokio::spawn(async move {
        axum::serve(listener, routes::build_router(&state))
            .await
            .expect("serve failure-path gateway");
    });
    ...
    let body: Value = response.json().await?;
        ...
        body.get("code").and_then(Value::as_str) == Some("upstream_unavailable"),
...
#[test]
fn phase10a3d_gateway_explore_surface_remains_proxy_only() -> TestResult {
    let source = std::fs::read_to_string("src/routes/product.rs")?;

    ensure(
        source.contains("FINAL_BETA_PHASE10A3D_SVC_GATEWAY_EXPLORE_DISCOVERY_ROUTE_V1"),
    ...
    let compact_source = source.split_whitespace().collect::<String>();
        ...
        compact_source.contains(".route(\"/explore\",get(explore_discovery))"),
        "public gateway Explore route missing",
    )?;

    ensure(
        source.contains("\"/v1/explore\""),
        "Omnigate Explore upstream missing",
    )?;

    ensure(
        source.contains("Omnigate owns the strict reviewed Explore query boundary"),
        "gateway versus Omnigate query authority boundary missing",
    )?;

    ensure(
        source.contains("read `svc-index` directly"),
...
fn parse_query(uri: &Uri) -> HashMap<String, String> {
    uri.query()
        ...
        .filter_map(|pair| {
            let pair = pair.split_once('=')?;

            Some((pair.0.to_owned(), pair.1.to_owned()))
        })
        .collect()
}
