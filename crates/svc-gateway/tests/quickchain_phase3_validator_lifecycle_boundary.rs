...
//! RO:WHAT — Phase 3 Round 2 validator lifecycle hardening boundary tests for svc-gateway.
//! RO:WHY — Gateway may proxy/display backend-derived lifecycle metadata, but must not become lifecycle, governance, paid-unlock, wallet, ledger, finality, staking, slashing, bridge, or settlement authority.
...
//! RO:SECURITY — blocks validator lifecycle/evidence/governance authority smuggling through public gateway routes and headers.
//! RO:TEST — cargo test -p svc-gateway --test quickchain_phase3_validator_lifecycle_boundary.
...
const LIFECYCLE_AUTHORITY_HEADERS: &[&str] = &[
...
#[test]
fn docs_name_phase3_round2_gateway_validator_lifecycle_boundary() {
...
#[test]
fn gateway_filters_phase3_validator_lifecycle_evidence_and_governance_authority_headers() {
    for raw in LIFECYCLE_AUTHORITY_HEADERS {
        let name = header(raw);

        assert!(
            !proxy::should_forward_passthrough_header(&name),
            "svc-gateway must not forward lifecycle authority passthrough header: {raw}"
        );
        assert!(
            !proxy::should_forward_product_header(&name),
            "svc-gateway must not forward lifecycle authority product header: {raw}"
        );
        assert!(
            !proxy::should_copy_response_header(&name),
            "svc-gateway must not copy lifecycle authority response header: {raw}"
        );
    }
}

#[test]
fn gateway_keeps_current_backend_derived_product_context_headers_but_not_authority_headers() {
    for raw in [
        "authorization",
        "accept",
        "content-type",
    ...
    for raw in [
        ...
        assert!(
            !proxy::should_forward_product_header(&name),
            "svc-gateway must reject authority-shaped lifecycle/governance header: {raw}"
        );
    }
}

#[test]
fn gateway_route_surface_does_not_expose_validator_lifecycle_mutation_routes() {
...
#[test]
fn gateway_source_does_not_construct_validator_lifecycle_or_paid_unlock_authority() {
...
#[test]
fn gateway_source_does_not_import_ledger_or_external_settlement_runtime() {
    let mut files = Vec::new();
