...
[ "$code" = "200" ] || { echo "gateway not ready"; tail -n 80 "$GLOG" || true; exit 1; }
...
echo
echo "===== 1) Fast path: /healthz ====="
bench "healthz" "$HEALTH_PATH"

echo
echo "===== 2) Guarded path: /readyz (no sleep, forced ready) ====="
bench "readyz" "$READY_PATH"

echo
echo "===== 3) Rejection hot path: /dev/rl (will produce 429s) ====="
bench "dev_rl" "$DEV_RL_PATH"

echo
echo "===== 4) Rejection hot path: /dev/echo (413 via body cap) ====="
# send a single too-large payload repeatedly; use curl for determinism
...
curl -s "http://${BIND_ADDR}/metrics" | grep -E 'gateway_http_requests_total|gateway_request_latency_seconds|gateway_rejections_total' | sed -n '1,120p' > "$LOG_DIR/metrics.txt"
sed -n '1,120p' "$LOG_DIR/metrics.txt"
