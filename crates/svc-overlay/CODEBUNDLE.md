<!-- Generated by scripts/make_crate_codex.sh on 2025-10-26T23:14:52Z -->
...
//!
//! RO:WHY
//!   Avoid "# no recorder" and avoid empty output by self-initializing minimal metrics.
//!
//! RO:INTERACTS
//!   - Other modules can bump overlay metrics via `overlay_metrics::*`.
//!
//! RO:INVARIANTS
//!
//! - Single global registry; encode errors become 500; zero panics.
//! - Text exposition is `text/plain; version=0.0.4` for Prometheus readers.
...
use prometheus::{
    default_registry, Encoder, Histogram, HistogramOpts, IntGauge, IntGaugeVec, Opts, Registry,
    TextEncoder,
};
use std::sync::Arc;
use tracing::warn;

static GLOBAL_REGISTRY: Lazy<Arc<Registry>> = Lazy::new(|| Arc::new(default_registry().clone()));

// --- Overlay golden metrics -------------------------------------------------
...
static OVERLAY_ACCEPT_LATENCY_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    let h = Histogram::with_opts(
        ...
        .buckets(vec![
            0.000_05, 0.000_1, 0.000_2, 0.000_5, 0.001, 0.002, 0.005, 0.01,
        ]),
    )
    .expect("histogram");
    GLOBAL_REGISTRY
...
// Always-on metric so /metrics is never empty.
...
/// Ensure that all metrics are registered and at least one sample exists.
/// Called on every /metrics request (cheap after first call).
fn ensure_registered() {
    // Touch the lazies to run their registration closures:
    let _ = &*OVERLAY_SESSIONS_ACTIVE;
    let _ = &*OVERLAY_ACCEPT_LATENCY_SECONDS;

    // Set build_info{version,git} = 1
...
pub mod overlay_metrics {
    ...
    pub fn inc_sessions_active() {
        super::OVERLAY_SESSIONS_ACTIVE.inc();
    }
    #[inline]
    pub fn dec_sessions_active() {
        super::OVERLAY_SESSIONS_ACTIVE.dec();
    }
    #[inline]
    pub fn observe_accept_latency_s(v: f64) {
        super::OVERLAY_ACCEPT_LATENCY_SECONDS.observe(v);
    }
}

/// GET /metrics
pub async fn handle_metrics() -> impl IntoResponse {
    // Make sure at least build_info exists and overlay gauges are registered.
    ensure_registered();
    ...
    let ctype = HeaderValue::from_static("text/plain; version=0.0.4");
    (StatusCode::OK, [(CONTENT_TYPE, ctype)], buf).into_response()
}
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn returns_non_empty_text() {
        let resp = handle_metrics().await.into_response();
        ...
        assert!(
            !body.is_empty(),
            "expected non-empty Prometheus exposition after ensure_registered()"
        );
    }
}
...
//! RO:NEXT — Swap TcpListener to ron-transport spawn once API is aligned.

use crate::admin::metrics::overlay_metrics;
use crate::admin::ReadyProbe;
use crate::config::Config;
use crate::conn::writer::write_frame;
use crate::gossip::publish; // publish Data frames into the minimal gossip engine
...
use bytes::BytesMut;
use std::net::SocketAddr;
use std::time::Instant; // std::time::Instant (avoid conflict with tokio::time)
use tokio::io::AsyncReadExt;
use tokio::net::TcpListener;
use tokio::task::JoinHandle;
...
// RAII timer to ensure we *always* record exactly one accept→(post-handshake/exit) latency.
struct AcceptTimer {
    t0: Instant,
    observed: bool,
}
impl AcceptTimer {
    ...
    fn observe_once(&mut self) {
        if !self.observed {
            overlay_metrics::observe_accept_latency_s(self.t0.elapsed().as_secs_f64());
            self.observed = true;
        }
    }
}
impl Drop for AcceptTimer {
    fn drop(&mut self) {
        // Guarantee a tick even on early-returns and error paths.
        self.observe_once();
    }
}
        ...
        loop {
            match listener.accept().await {
                Ok((sock, peer)) => {
                    ...
                    tokio::spawn(handle_conn(sock, peer).in_current_span());
                }
                Err(e) => {
...
async fn handle_conn(mut sock: tokio::net::TcpStream, peer: SocketAddr) {
    info!(%peer, "conn: accepted");

    // Start latency timer (observed in Drop if we forget)
    let mut accept_timer = AcceptTimer::start();

    // Minimal symmetric hello (OAP/1). Timeout bounds resource usage.
    let caps = Caps::GOSSIP_V1;
    match handshake(&mut sock, caps, Duration::from_secs(3)).await {
        Ok(neg) => {
            // Observe accept→post-handshake latency
            accept_timer.observe_once();
            info!(%peer, ver = neg.version, caps = ?neg.caps, "conn: negotiated");
        }
        Err(e) => {
            // Let Drop record latency; still count failure reason
            let reason = match &e {
                crate::protocol::error::ProtoError::HandshakeTimeout => "timeout",
                crate::protocol::error::ProtoError::CapabilityMismatch => "caps",
                crate::protocol::error::ProtoError::BadPreamble { .. } => "preamble",
                crate::protocol::error::ProtoError::Io(_) => "io",
                _ => "other",
            };
            metrics::counter!("overlay_handshake_fail_total", "reason" => reason).increment(1);
            warn!(%peer, error=?e, "conn: handshake failed");
            overlay_metrics::dec_sessions_active();
            return; // Drop fires here → latency observed
        }
    }

    // Echo loop with metrics.
    ...
    loop {
        while let Some(frame) = match try_parse_frame(&mut inbuf) {
            ...
            Err(e) => {
                ...
                overlay_metrics::dec_sessions_active();
                return; // Drop fires → latency observed (already observed post-handshake)
            }
        } {
            match frame.kind {
                FrameKind::Data => {
                    let len = frame.payload.len() as u64;
                    debug!(%peer, len, "oap/data in");
                    metrics::counter!("overlay_frames_in_total", "kind" => "data").increment(1);
                    metrics::counter!("overlay_bytes_in_total").increment(len);

                    // Publish into the gossip engine (best effort).
                    let _published = publish(frame.payload.clone());

                    let echo = Frame {
                        kind: FrameKind::Data,
                        payload: frame.payload,
                    };
                    if let Err(e) = write_frame(&mut sock, &echo, &mut outbuf).await {
                        error!(%peer, error=?e, "conn: echo write failed");
                        overlay_metrics::dec_sessions_active();
                        return; // Drop fires (already observed)
                    }
                    metrics::counter!("overlay_frames_out_total", "kind" => "data").increment(1);
                    metrics::counter!("overlay_bytes_out_total").increment(len);
                    debug!(%peer, "oap/data echoed");
                }
                FrameKind::Ctrl => {
                    let len = frame.payload.len() as u64;
                    metrics::counter!("overlay_frames_in_total", "kind" => "ctrl").increment(1);
                    metrics::counter!("overlay_bytes_in_total").increment(len);
                    debug!(%peer, len, "oap/ctrl in (ignored)");
                }
            }
            trace!(%peer, buf_len = inbuf.len(), "post-parse buffer");
        }

        match sock.read_buf(&mut inbuf).await {
            Ok(0) => {
                let secs = accept_timer.t0.elapsed().as_secs_f64();
                metrics::histogram!("overlay_conn_lifetime_seconds").record(secs);
