...
#[tokio::test]
async fn oap_roundtrip_ack_and_bus_events() {
    ...
    let (handle, bound) = srv.serve("127.0.0.1:0".parse().unwrap()).await.unwrap();
    ...
    let df = data_frame(json!({"mime":"text/plain"}), body, DEFAULT_MAX_FRAME).unwrap();
    ...
    // END stream
    write_frame(&mut s, &end_frame(), DEFAULT_MAX_FRAME).await.unwrap();
