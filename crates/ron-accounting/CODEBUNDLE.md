<!-- Generated by scripts/make_crate_codex.sh on 2026-06-26T04:58:35Z -->
...
/// receipt, not ledger state, not payout side effect, and not settlement/finality.
...
/// not ledger truth, not payout authority, not settlement/finality, and not a
...
pub struct QuickChainBondDisputeReport {
    ...
    /// Must remain false: accounting does not execute payouts.
    pub payout_side_effect: bool,
    /// Must remain false: accounting does not create finality truth.
    pub finality_truth: bool,
    /// Must remain false: accounting does not create settlement truth.
    pub settlement_truth: bool,
}
...
impl QuickChainBondDisputeReport {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.finality_truth {
            return Err(Error::schema(
                "bond dispute report must not claim finality truth",
            ));
        }

        if self.settlement_truth {
            return Err(Error::schema(
                "bond dispute report must not claim settlement truth",
            ));
        }
...
fn dispute_report_rejects_authority_flags_and_inconsistent_state() {
    ...
    let mut finality_truth = valid_report();
    finality_truth.finality_truth = true;
    assert!(
        finality_truth.validate().is_err(),
        "accounting dispute report must not claim finality truth"
    );

    let mut settlement_truth = valid_report();
    settlement_truth.settlement_truth = true;
    assert!(
        settlement_truth.validate().is_err(),
        "accounting dispute report must not claim settlement truth"
    );
