<!-- Generated by scripts/make_crate_codex.sh on 2025-11-02T19:12:27Z -->
...
API_ADDR="${API_ADDR:-127.0.0.1:5305}"
ADMIN_ADDR="${ADMIN_ADDR:-127.0.0.1:9605}"
# Size used for the 413 test; default aligns with your Admission default (10 MiB)
MAX_BODY="${OMNIGATE_MAX_BODY:-10485760}"

say() { printf "\n\033[1m▶ %s\033[0m\n" "$*"; }

# Print only the status line to stderr, body to stdout
http() { curl -sS -D >(sed -n '1,1p' >&2) -o /tmp/resp.$$ "$@"; cat /tmp/resp.$$; rm -f /tmp/resp.$$; }
status() { curl -s -o /dev/null -w "%{http_code}" "$@"; }

metric_sum() {
  # Sum all samples for a metric (accept either UPPERCASE or lowercase variant)
  local name_uc="$1"
  local name_lc
  name_lc="$(printf "%s" "$1" | tr '[:upper:]' '[:lower:]')"

  curl -s "http://${ADMIN_ADDR}/metrics" \
  | awk -v n1="^"$(printf "%s" "$name_uc" | sed 's/[].[^$*+?{}()|/\\]/\\&/g') \
        -v n2="^"$(printf "%s" "$name_lc" | sed 's/[].[^$*+?{}()|/\\]/\\&/g') '
      $0 ~ n1 || $0 ~ n2 {
        v = $NF           # last field is the numeric value in Prometheus exposition
        if (v ~ /^[0-9]+([.][0-9]+)?$/) sum += v + 0
      }
      END { printf("%.0f\n", (sum == "" ? 0 : sum)) }'
}

assert_status() {
  local have="$1" want="$2" label="$3"
  if [[ "$have" != "$want" ]]; then
    echo "❌ ${label}: expected ${want}, got ${have}"
    exit 1
  else
    echo "✅ ${label}: ${have}"
  fi
...
say "Check /readyz"
assert_status "$(status "http://${ADMIN_ADDR}/readyz")" "200" "/readyz"

say "GET /v1/ping (expect 200)"
assert_status "$(status -X GET "http://${API_ADDR}/v1/ping")" "200" "GET /v1/ping"

# Read counters before we fire tests
P_BEFORE="$(metric_sum POLICY_SHORTCIRCUITS_TOTAL)"
B_BEFORE="$(metric_sum BODY_REJECT_TOTAL)"
D_BEFORE="$(metric_sum DECOMPRESS_REJECT_TOTAL)"

say "PUT /v1/ping with Content-Length: 0 (expect policy 403)"
assert_status "$(status -X PUT -H "Content-Length: 0" "http://${API_ADDR}/v1/ping")" "403" "PUT /v1/ping (CL:0)"

say "PUT /v1/ping with stacked encodings (expect 415)"
assert_status "$(status -X PUT \
  -H "Content-Encoding: gzip, br" \
  -H "Content-Length: 0" \
  "http://${API_ADDR}/v1/ping")" "415" "PUT /v1/ping (stacked encodings)"

say "PUT /v1/ping oversize body (expect 413)"
TMP_BIG="$(mktemp -t omnigate-big.XXXXXX)"
# macOS: head -c works; fallback to dd if not
if head -c "$((MAX_BODY + 1))" /dev/zero > "$TMP_BIG" 2>/dev/null; then :; else
  dd if=/dev/zero bs=1 count=$((MAX_BODY + 1)) of="$TMP_BIG" status=none
fi
assert_status "$(status -X PUT --data-binary @"$TMP_BIG" "http://${API_ADDR}/v1/ping")" "413" "PUT /v1/ping (oversize)"
rm -f "$TMP_BIG"

# Read counters after
P_AFTER="$(metric_sum POLICY_SHORTCIRCUITS_TOTAL)"
B_AFTER="$(metric_sum BODY_REJECT_TOTAL)"
D_AFTER="$(metric_sum DECOMPRESS_REJECT_TOTAL)"

say "Metrics deltas"
printf "  POLICY_SHORTCIRCUITS_TOTAL: %s -> %s  (Δ=%s)\n" "$P_BEFORE" "$P_AFTER" "$((P_AFTER - P_BEFORE))"
printf "  BODY_REJECT_TOTAL:          %s -> %s  (Δ=%s)\n" "$B_BEFORE" "$B_AFTER" "$((B_AFTER - B_BEFORE))"
printf "  DECOMPRESS_REJECT_TOTAL:    %s -> %s  (Δ=%s)\n" "$D_BEFORE" "$D_AFTER" "$((D_AFTER - D_BEFORE))"
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    // clippy(let_and_return): return the expression directly
...
/// Attach the quota limiter layer to the Router (default constants).
/// NOTE: retained for tests/back-compat; prefer `attach_with_cfg`.
pub fn attach<S>(router: Router<S>) -> Router<S>
where
    S: Clone + Send + Sync + 'static,
{
    // Existing behavior as in your bundle: 500 qps / 1000 burst.
    // (We keep this for back-compat; config-aware attach is below.)
    // Ref: CODEBUNDLE.md shows this exact default attach shape.
    let limiter = GlobalLimiter::new(500, 1000);
    router.layer(from_fn_with_state(limiter, quota_guard))
}

/// Attach the quota limiter layer using GlobalQuota from Config.
pub fn attach_with_cfg<S>(router: Router<S>, gq: &crate::config::GlobalQuota) -> Router<S>
where
    S: Clone + Send + Sync + 'static,
{
    let (qps, burst) = gq.params_u32(); // from Config helpers per NOTES
...
///
/// NOTE: Return a concrete `Response` (not `impl IntoResponse`) to avoid E0700
/// when `message` is an `&str` with an anonymous lifetime.
...
//!
//! Also hosts gate-level guard counters used by policy/body/decompress middleware
//! so everything exports from the default Prometheus registry.
...
/// Label `code` is the HTTP status code as a string: "403" | "451" | "503".
pub static POLICY_SHORTCIRCUITS_TOTAL: Lazy<IntCounterVec> = Lazy::new(|| {
    ...
    .expect("register policy_shortcircuits_total")
});

/// Body admission rejections.
/// Label `reason`: "oversize" | "missing_length".
...
/// Label `reason`: "stacked" | "unknown" | "over_budget".
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    // NOTE: decompression is cfg-driven; the module exposes cfg-aware attach (no plain `layer()`).
    // This aligns with our notes to wire admission/body/decompress from config.
...
use crate::metrics::gates::POLICY_SHORTCIRCUITS_TOTAL;
