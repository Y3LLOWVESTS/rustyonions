<!-- Generated by scripts/make_crate_codex.sh on 2025-11-01T18:33:21Z -->
    ...
    S::Error: Into<Box<dyn std::error::Error + Send + Sync>> + Send + 'static,
{
    ...
    fn call(&mut self, mut req: Request) -> Self::Future {
        ...
        // Pull an optional ron-policy evaluator from request extensions.
        // This avoids depending on crate::state::AppState while allowing
        // callers to inject `extensions.insert(Arc<Evaluator>)` when ready.
        let maybe_eval = req.extensions().get::<Arc<ron_policy::Evaluator>>().cloned();

        Box::pin(async move {
            if let Some(eval) = maybe_eval {
                // Build the minimal Context that ron-policy v0.1.0-beta.1 expects.
                // (This type currently exposes { body_bytes, now_ms } – no path/method.)
                ...
                match eval.evaluate(&ctx) {
                    Ok(dec) if dec.effect.is_allow() => {
                        // Allowed → pass through.
                        let res = inner.call(req).await?;
                        return Ok(res.into_response());
                    }
                    Ok(dec) => {
                        // Deny (403 by default; 451 if reason="LEGAL").
            ...
            // No evaluator present → no-op pass-through.
            let res = inner.call(req).await?;
...
pub async fn search_echo(Json(body): Json<SearchRequest>) -> Json<SearchReply> {
    Json(SearchReply { ok: true, echoed: body.q })
}
...
```rust
//! RO:WHAT   v1: /mailbox health stub (client passthrough soon).
//! RO:INVARS 200/JSON only; no internals.
