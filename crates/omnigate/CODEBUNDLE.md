<!-- Generated by scripts/make_crate_codex.sh on 2025-11-03T17:37:22Z -->
...
struct AdminState {
    ...
    max_inflight_threshold: i64,
    error_rate_429_503_pct: f64,
    hold_for_secs: u64,
    // Degrade hold (until this instant we stick to 503)
...
impl App {
    ...
    pub async fn build(cfg: config::Config) -> anyhow::Result<Self> {
        ...
        let admin_state = AdminState {
            ...
            ready: ready.clone(),
            dev_ready,
            max_inflight_threshold: cfg.readiness.max_inflight_threshold as i64,
            error_rate_429_503_pct: cfg.readiness.error_rate_429_503_pct,
            hold_for_secs: cfg.readiness.hold_for_secs,
        ...
        // Local readiness policy wrapper (trips based on our gauges + holds for N seconds).
        async fn readyz(State(st): State<AdminState>) -> impl IntoResponse {
            use crate::errors::http_map::GateError;
            use crate::metrics::gates::{
                READY_ERROR_RATE_PCT, READY_INFLIGHT_CURRENT, READY_STATE_CHANGES_TOTAL,
                READY_TRIPS_TOTAL,
            };
            ...
            let now = Instant::now();
            if let Some(until) = *st.hold_until.lock().expect("hold_until mutex poisoned") {
                if now < until {
                    return GateError::Degraded.into_response();
                } else {
                    // Hold expired — clear and mark recovery.
                    *st.hold_until.lock().unwrap() = None;
            ...
            // Sample current gauges from our admission/fair-queue wiring.
            let inflight = READY_INFLIGHT_CURRENT.get();
            let err_pct_like = READY_ERROR_RATE_PCT.get();
            ...
            if trip_inflight || trip_err {
                ...
                let until = now + Duration::from_secs(st.hold_for_secs.max(1));
                *st.hold_until.lock().unwrap() = Some(until);
        ...
        // -------------------- READINESS SAMPLER (metrics-only; gate reads the gauges) --------------------
