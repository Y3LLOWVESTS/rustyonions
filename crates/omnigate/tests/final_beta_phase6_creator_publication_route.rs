...
async fn phase6b3_upstream_not_found_passes_through() {
    ...
    let response =
        reqwest::Client::new()
            ...
            .await
            .expect(
                "publication not-found response",
            );
...
async fn phase6b3_invalid_path_segments_fail_before_upstream() {
    ...
    let invalid_username =
        client
            .get(
                format!(
                    "http://{address}/v1/creators/BadUser/publications",
                ),
            )
            .send()
            .await
...
fn phase6b3_route_surface_adds_no_economic_or_relationship_authority() {
    let route_source =
        include_str!(
            "../src/routes/v1/creator_publications.rs",
        );
