...
//! RO:WHAT — QC-1A pair-interlock tests for omnigate.
//! RO:WHY — Keeps hydration/access composition from becoming wallet, ledger, root, finality, validator, bridge, or settlement authority.
//! RO:INTERACTS — docs/quickchain-preflight.md, scripts/dev-quickchain-preflight.sh, omnigate runtime source.
//! RO:INVARIANTS — backend-derived display/access only; cache/display metadata cannot unlock; no direct ledger mutation.
//! RO:METRICS — none.
//! RO:CONFIG — none.
//! RO:SECURITY — blocks authority creep through hydration, routes, source shortcuts, or cache/display receipt trust.
//! RO:TEST — cargo test -p omnigate --test quickchain_preflight_phase1_pair_interlock.
...
#[test]
fn docs_lock_omnigate_into_qc1a_pair_interlock_role() {
...
fn runtime_source_does_not_define_direct_economic_or_quickchain_authority() {
    ...
    assert_contains_none(
        ...
        &[
            ...
            "unlock_from_display_receipt",
            "unlock_from_hydration_cache",
            "unlock_from_index_pointer",
            "unlock_from_policy_decision",
            "anchor_lang",
            "solana_sdk",
            "solana_client",
            "spl_token",
