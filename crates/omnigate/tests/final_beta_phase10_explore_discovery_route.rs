    ...
    (
        ...
        Json(
            ErrorFixture {
                ...
                message:
                    "discovery projection unavailable",
            },
        ),
    )
}
...
#[tokio::test]
async fn phase10a3c_defaults_are_forwarded_to_svc_index(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[tokio::test]
async fn phase10a3c_explicit_bounded_limits_are_forwarded_exactly(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[tokio::test]
async fn phase10a3c_out_of_bound_limit_fails_before_upstream_activity(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[tokio::test]
async fn phase10a3c_unknown_query_field_fails_closed(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[tokio::test]
async fn phase10a3c_success_body_matches_svc_index_wire_shape(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[tokio::test]
async fn phase10a3c_upstream_status_and_body_pass_through(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[tokio::test]
async fn phase10a3c_transport_failure_returns_stable_bad_gateway(
) -> TestResult {
    let _guard =
        ENV_LOCK.lock().await;
...
#[test]
fn phase10a3c_route_surface_is_read_only_and_authority_free(
) -> TestResult {
    let route_source =
        std::fs::read_to_string(
            "src/routes/v1/explore_discovery.rs",
        )?;
    ...
    let compact_router =
        ...
        compact_router.contains(
            "explore_discovery::get_explore_discovery",
        ),
