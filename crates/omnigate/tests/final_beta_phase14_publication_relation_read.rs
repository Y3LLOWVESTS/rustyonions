...
async fn start_dummy_index() -> SocketAddr {
    ...
    async fn relation_handler(
        ...
        if query.get("parentCrabUrl").is_none() {
            return (
    ...
    let router = Router::new()
        ...
        .route("/v1/index/publication-relations", get(relation_handler));
    ...
    tokio::spawn(async move {
        axum::serve(listener, router)
            .await
            .expect("serve dummy relation index");
    });
...
async fn phase14a6d_omnigate_proxies_parent_cursor_limit_and_headers() {
    ...
    let response = reqwest::Client::new()
        ...
        .query(&[
            ("parentCrabUrl", IMAGE_URL),
            ("cursor", "r_00000001"),
            ("limit", "25"),
        ])
        .header("authorization", "Bearer relation-read")
        .header("x-correlation-id", "corr-phase14a6d")
        .header("x-request-id", "req-phase14a6d")
        .header("connection", "close")
        .send()
        .await
        .expect("omnigate relation read");

    assert_eq!(response.status(), StatusCode::OK,);

    let body: Value = response.json().await.expect("relation page JSON");

    assert_eq!(body["schema"], "crablink.publication-relation-page.v1",);

    assert_eq!(body["items"][0]["parentCrabUrl"], IMAGE_URL,);

    assert_eq!(body["items"][0]["threadCrabUrl"], IMAGE_URL,);

    assert_eq!(body["nextCursor"], "r_00000001",);

    assert_eq!(body["receivedLimit"], "25",);

    assert_eq!(body["authorization"], "Bearer relation-read",);

    assert_eq!(body["xCorrelationId"], "corr-phase14a6d",);

    assert_eq!(body["xRequestId"], "req-phase14a6d",);
...
async fn phase14a6d_unknown_query_fields_fail_before_index() {
    ...
    let response = reqwest::Client::new()
        .get(format!("http://{address}/v1/publication-relations",))
        .query(&[("parentCrabUrl", IMAGE_URL), ("walletBalance", "100")])
        .send()
        .await
        .expect("unknown relation query response");
...
async fn phase14a6d_parent_query_is_required() {
    ...
    let response = reqwest::Client::new()
        ...
        .send()
        .await
        .expect("missing relation parent response");
...
async fn phase14a6d_index_transport_failure_is_structured_bad_gateway() {
    ...
    let response = reqwest::Client::new()
        .get(format!("http://{address}/v1/publication-relations",))
        .query(&[("parentCrabUrl", IMAGE_URL)])
        .send()
        .await
        .expect("relation transport response");

    assert_eq!(response.status(), StatusCode::BAD_GATEWAY,);

    let body: Value = response.json().await.expect("relation bad gateway JSON");
...
fn phase14a6d_omnigate_route_surface_is_read_only_proxy() {
    let route_source = include_str!("../src/routes/v1/publication_relations.rs",);
