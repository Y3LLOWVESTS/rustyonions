...
//! RO:WHAT — Phase 1 Round 2 downstream-confirmation tests for omnigate.
//! RO:WHY — Confirms omnigate remains product hydration/access coordination while core QuickChain roots/proofs stay elsewhere.
//! RO:INTERACTS — docs/quickchain-preflight.md, scripts/dev-quickchain-preflight.sh, route/hydration/wallet coordination sources.
//! RO:INVARIANTS — omnigate does not become wallet, ledger, root, checkpoint, validator, bridge, or finality authority.
...
//! RO:TEST — cargo test -p omnigate --test quickchain_phase1_round2_confirmation.
...
#[test]
fn docs_name_phase1_round2_omnigate_downstream_confirmation_boundary() {
...
fn docs_separate_accepted_receipt_from_future_finality_labels() {
    ...
    for required in [
        ...
        assert_contains(&doc, required, "omnigate accepted/finality docs");
    }
}
...
fn source_has_no_direct_ledger_or_quickchain_authority_imports() {
    ...
    for forbidden in [
        ...
        assert_not_contains(&source, forbidden, "omnigate src");
    }
}
    ...
    for literal in string_literals(&source) {
        ...
        for forbidden in forbidden_route_prefixes {
            assert!(
                ...
                "omnigate must not expose forbidden QuickChain/external route literal `{literal}`"
            );
        }
    }
}
