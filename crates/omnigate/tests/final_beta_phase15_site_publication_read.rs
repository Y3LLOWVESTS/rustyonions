...
async fn start_dummy_index() -> SocketAddr {
    ...
    let listener =
        ...
        .await
        .expect(
            "bind dummy Site-publication index",
        );
    ...
    tokio::spawn(
        async move {
            ...
            .await
            .expect(
                "serve dummy Site-publication index",
            );
        },
    );
...
async fn phase15a4a2c1_site_publication_page_proxies_index_truth() {
    ...
    let response =
        reqwest::Client::new()
            .get(
                format!(
                    "http://{address}/v1/site-publications",
                ),
            )
            .query(
                &[
            ...
            .await
            .expect(
                "Omnigate Site-publication read",
            );
    ...
    assert_eq!(
        body["items"][0]["crabUrl"],
        POST_URL,
    );

    assert_eq!(
        body["items"][0]["siteCrabUrl"],
        SITE_URL,
    );

    assert_eq!(
        body["items"][0]["creatorDisplay"],
        "@alice",
    );
...
async fn phase15a4a2c1_unknown_query_fields_fail_before_index() {
    ...
    let address =
        start_omnigate_with_current_env()
            .await;
    ...
    let response =
        reqwest::Client::new()
            ...
            .await
            .expect(
                "unknown Site-publication query response",
            );
...
async fn phase15a4a2c1_site_query_is_required() {
    ...
    let response =
        reqwest::Client::new()
            .get(
                format!(
                    "http://{address}/v1/site-publications?limit=20",
                ),
            )
            .send()
            .await
            .expect(
                "missing Site-publication Site response",
            );
...
async fn phase15a4a2c1_index_transport_failure_is_structured_bad_gateway() {
    ...
    let response =
        reqwest::Client::new()
            ...
            .await
            .expect(
                "Site-publication transport response",
            );
    ...
    let body: Value =
        response
            ...
            .await
            .expect(
                "Site-publication bad gateway JSON",
            );
...
fn phase15a4a2c1_route_surface_is_generic_read_only_proxy() {
    let route_source =
        include_str!(
            "../src/routes/v1/site_publications.rs",
        );
