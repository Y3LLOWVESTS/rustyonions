//! text_asset_publish.rs — integration tests for `/v1/assets/post/*`.
//!
//! RO:WHAT — Spin up dummy svc-storage/index and real omnigate routes; assert post prepare/publish behavior.
//! RO:WHY — NEXT_LEVEL starts text assets with post before comment/article so the b3+manifest+index pattern is proven one primitive at a time.
...
//! RO:TEST — cargo test -p omnigate --test text_asset_publish.
...
const POST_ASSET_CID: &str = "b3:1111111111111111111111111111111111111111111111111111111111111111";
...
async fn start_dummy_storage() -> SocketAddr {
        ...
        if bytes == 13 {
            return (
                ...
                Json(serde_json::json!(ErrorBody {
                    ...
                    reason: "storage estimate rejected post fixture",
                })),
            );
        }
    ...
    async fn paid_upload_handler(headers: HeaderMap, body: Bytes) -> (StatusCode, Json<Value>) {
        ...
        let content: Value =
            serde_json::from_slice(&body).expect("omnigate should send post content JSON");

        assert_eq!(content["schema"], "ron.post-content.v1");
        assert_eq!(content["asset_kind"], "post");
        assert_eq!(content["title"], "First backend post");
        assert_eq!(
            content["body"],
            "This post should become a b3-backed text asset."
        );
        assert_eq!(
            content["site_connection"]["crab_url"],
        ...
        (
            ...
            Json(serde_json::json!(UploadEcho {
                ...
                wallet_txid: "tx_paid_post_1",
                wallet_receipt_hash: "receipt_hash_paid_post_1",
                estimate_minor: "25",
                body_len: body.len(),
    ...
    async fn put_object_handler(headers: HeaderMap, body: Bytes) -> (StatusCode, Json<Value>) {
        ...
        let manifest: Value =
            serde_json::from_slice(&body).expect("omnigate should send post manifest JSON");

        assert_eq!(manifest["version"], 1);
        assert_eq!(manifest["asset_cid"], POST_ASSET_CID);
        assert_eq!(manifest["asset_kind"], "post");
        ...
        assert_eq!(manifest["metadata"]["title"], "First backend post");
        assert_eq!(manifest["metadata"]["tags"][0], "post");
        ...
        assert_eq!(
            manifest["parent_reference"]["crab_url"],
            "crab://3333333333333333333333333333333333333333333333333333333333333333.post"
        );
        assert_eq!(manifest["receipts"][0]["tx_id"], "tx_paid_post_1");
...
async fn start_dummy_index() -> SocketAddr {
    ...
    async fn put_asset_pointer(
        ...
        assert_eq!(body.asset_kind, "post");
        assert_eq!(body.manifest_cid, POST_MANIFEST_CID);
        assert_eq!(
...
#[tokio::test]
async fn post_asset_prepare_returns_hold_template() {
    ...
    let resp = client
        .post(format!("http://{omnigate_addr}/v1/assets/post/prepare"))
        .header("authorization", "Bearer dev")
        .header("x-ron-passport", "passport:main:alice")
        .header("x-ron-wallet-account", "acct_creator_alice")
        .header("idempotency-key", "idem-post-prepare-1")
        .header("connection", "close")
        .json(&request)
        .send()
        .await
        .expect("omnigate post prepare response");

    assert_eq!(resp.status(), StatusCode::OK);

    let body: Value = resp.json().await.expect("parse post prepare JSON body");

    assert_eq!(body["schema"], "omnigate.post-asset-prepare.v1");
    assert_eq!(body["asset_kind"], "post");
    ...
    assert_eq!(
        body["parent_reference"]["crab_url"],
        "crab://3333333333333333333333333333333333333333333333333333333333333333.post"
    );
    ...
    assert_eq!(body["next"]["submit_publish"], "/v1/assets/post");
    assert_eq!(
        body["next"]["resolve_after_publish"],
        "/v1/crab/resolve?url=crab://<hash>.post"
    );
...
#[tokio::test]
async fn post_asset_prepare_requires_site_context() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
    ...
    request["site_context_crab_url"] = Value::Null;
    ...
    let resp = client
        ...
        .send()
        .await
        .expect("omnigate post prepare validation response");

    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);

    let body: Value = resp.json().await.expect("parse validation JSON body");
    assert_eq!(body["code"], "invalid_post_prepare_request");
    assert_eq!(body["reason"], "missing_site_context");

    clear_env();
}

#[tokio::test]
async fn post_asset_publish_coordinates_paid_storage_manifest_and_index_pointer() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
    ...
    let client = reqwest::Client::new();
    let resp = client
        .post(format!("http://{omnigate_addr}/v1/assets/post"))
        .header("authorization", "Bearer dev")
        .header("content-type", "application/json")
        .header("x-ron-paid-op", "hold")
        .header("x-ron-paid-asset", "roc")
        .header("x-ron-paid-estimate-minor", "25")
        .header("x-ron-wallet-txid", "tx_paid_post_1")
        .header("x-ron-wallet-receipt-hash", "receipt_hash_paid_post_1")
        .header("x-ron-wallet-from", "acct_creator_alice")
        .header("x-ron-wallet-to", "escrow_paid_write")
        .header("x-ron-asset-kind", "post")
        .header("x-ron-passport", "passport:main:alice")
        .header("x-ron-wallet-account", "acct_creator_alice")
        .header("idempotency-key", "idem-post-publish-1")
        .header("connection", "close")
        .json(&post_request())
        .send()
        .await
        .expect("omnigate post publish coordinator response");

    assert_eq!(resp.status(), StatusCode::OK);

    let body: Value = resp.json().await.expect("parse post publish JSON body");

    assert_eq!(body["schema"], "omnigate.post-asset-publish.v1");
    assert_eq!(body["asset_kind"], "post");
    assert_eq!(body["asset_cid"], POST_ASSET_CID);
    assert_eq!(
        body["crab_url"],
        "crab://1111111111111111111111111111111111111111111111111111111111111111.post"
    );

    assert_eq!(body["storage_upload"]["cid"], POST_ASSET_CID);
    ...
    assert_eq!(body["storage_upload"]["x_ron_asset_kind"], "post");
    ...
    assert_eq!(body["manifest"]["manifest_cid"], POST_MANIFEST_CID);
    assert_eq!(body["manifest"]["storage_path"], "/o");

    assert_eq!(body["index_pointer"]["status"], "stored");
    assert_eq!(
        body["index_pointer"]["route"],
        "/v1/index/assets/1111111111111111111111111111111111111111111111111111111111111111/manifest"
    );
    ...
    assert_eq!(body["links"]["raw"], format!("/o/{POST_ASSET_CID}"));
    assert_eq!(
        body["links"]["crab"],
        "crab://1111111111111111111111111111111111111111111111111111111111111111.post"
    );
    assert_eq!(
        body["links"]["http_b3"],
        "/v1/b3/1111111111111111111111111111111111111111111111111111111111111111.post"
    );
    assert_eq!(
        body["links"]["resolve"],
        "/v1/crab/resolve?url=crab://1111111111111111111111111111111111111111111111111111111111111111.post"
    );
    assert_eq!(
        body["links"]["manifest_raw"],
        format!("/o/{POST_MANIFEST_CID}")
    );
...
#[tokio::test]
async fn post_asset_publish_requires_paid_proof_headers() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
    ...
    let resp = client
        ...
        .send()
        .await
        .expect("omnigate post publish missing proof response");
...
#[tokio::test]
async fn post_asset_prepare_upstream_connect_failure_yields_problem_502() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
    ...
    let client = reqwest::Client::new();
    let resp = client
        .post(format!("http://{addr}/v1/assets/post/prepare"))
        .json(&post_request())
        .send()
        .await
        .expect("omnigate post prepare response");
...
fn post_request() -> Value {
    serde_json::json!({
        "title": "First backend post",
        "body": "This post should become a b3-backed text asset.",
        "creator_display": "@alice",
        "language": "en",
        "post_kind": "thread_starter",
        ...
        "owner_passport_subject": "passport:main:alice",
        "client_idempotency_key": "idem-post-prepare-1"
    })
}
