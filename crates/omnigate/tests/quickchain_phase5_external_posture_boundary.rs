...
//! RO:WHAT — Phase 5 Round 3 selected external-posture boundary tests for omnigate.
//! RO:WHY — Omnigate may hydrate selected external posture status/evidence as display metadata only,
...
//! RO:INTERACTS — docs/quickchain-preflight.md, routes/v1/header_policy.rs, route/hydration/middleware source.
//! RO:INVARIANTS — external posture hydration is evidence-only/status-only; wallet/ledger remain truth.
//! RO:METRICS — none; source/docs/header-policy boundary test.
//! RO:CONFIG — source-only checks.
//! RO:SECURITY — blocks selected external posture authority smuggling through hydration/access composition.
//! RO:TEST — cargo test -p omnigate --test quickchain_phase5_external_posture_boundary.
...
const HYDRATION_SCHEMA: &str = "omnigate.quickchain-external-posture-hydration.v1";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct HydratedExternalPostureStatus {
    schema: String,
    produced_at_ms: u64,
...
impl HydratedExternalPostureStatus {
    fn sample() -> Self {
        Self {
            schema: HYDRATION_SCHEMA.to_string(),
            produced_at_ms: 1_777_700_004_000,
            hydration_source: "backend-derived:omnigate".to_string(),
            chain_id: "roc-dev".to_string(),
            epoch_id: "epoch:phase5:r3:omnigate".to_string(),
            posture_id: "external-posture:phase5:r3:omnigate".to_string(),
            display_label: "external_posture_display_only_evidence".to_string(),
            selected_posture: "anchor-only".to_string(),
            anchor_commitment: format!("b3:{}", "2".repeat(64)),
            evidence_cid: format!("b3:{}", "3".repeat(64)),
            report_cid: format!("b3:{}", "4".repeat(64)),
    ...
    fn validate(&self) -> Result<(), String> {
        if self.schema != HYDRATION_SCHEMA {
            return Err("invalid omnigate external posture hydration schema".to_string());
        }
        ...
        if self.display_label != "external_posture_display_only_evidence" {
            return Err("omnigate external posture label must remain display-only".to_string());
        }

        if self.selected_posture != "anchor-only" {
            return Err("omnigate selected posture must remain anchor-only".to_string());
        }
        ...
        for (name, value) in [
            ...
            if !value {
                return Err(format!(
                    "{name} must be true for omnigate external posture hydration"
                ));
            }
        }
        ...
        for (name, value) in [
            ...
            if value {
                return Err(format!(
                    "{name} must remain false for omnigate external posture hydration"
                ));
            }
        }
    ...
    match value {
        Value::Object(object) => {
            for (key, nested) in object {
                assert_ne!(
                    key, forbidden,
                    "omnigate external posture must not expose forbidden authority key `{forbidden}`"
                );
...
#[test]
fn docs_name_phase5_round3_omnigate_external_posture_boundary() {
...
#[test]
fn omnigate_external_posture_hydration_is_display_only_evidence_metadata() {
    let report = HydratedExternalPostureStatus::sample();
    report
        .validate()
        .expect("external posture hydration should validate");
...
#[test]
fn omnigate_external_posture_rejects_authority_flags_unknown_fields_and_bad_b3() {
    for field in [
        "external_settlement_enabled",
        ...
        let mut value = serde_json::to_value(HydratedExternalPostureStatus::sample())
        ...
        let parsed: HydratedExternalPostureStatus =
    ...
    for poison in [
        "external_posture_hydration_unlock",
        "unlock_from_external_posture",
        "receipt_truth_from_external_posture",
        "balance_truth_from_external_posture",
        "settlement_truth_from_external_posture",
        "finality_truth_from_external_posture",
        "reward_truth_from_external_posture",
        "wallet_mutation",
        "ledger_mutation",
        "paid_unlock",
        "balance_minor",
...
#[test]
fn omnigate_header_policy_names_phase5_external_posture_authority_families() {
...
#[test]
fn omnigate_route_surface_does_not_expose_phase5_external_posture_runtime_routes() {
...
#[test]
fn omnigate_source_does_not_construct_phase5_external_posture_runtime_authority() {
    ...
    collect_rust_files(&crate_root().join("src/downstream"), &mut files);
    ...
    let forbidden_compact_markers = [
        ...
        "omnigate_external_posture_truth:true",
        "omnigate_outside_program_authority:true",
        "hydrate_as_external_posture_truth",
...
#[test]
fn existing_paid_and_access_routes_do_not_unlock_from_external_posture_material() {
    for rel in [
...
#[test]
fn omnigate_does_not_gain_external_settlement_runtime_dependencies() {
