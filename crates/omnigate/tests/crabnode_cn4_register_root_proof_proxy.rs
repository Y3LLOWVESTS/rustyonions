//! RO:WHAT — CN-4 acceptance tests for Omnigate's fixed RegisterRoot proof proxy.
//! RO:WHY — Prove the reviewed proof path reaches only svc-passport's fixed RegisterRoot proof endpoint.
//! RO:INTERACTS — Omnigate v1 router, Passport proxy, dummy loopback svc-passport.
//! RO:INVARIANTS — exact path/body forwarding; 16 KiB cap; nearby unreviewed prove routes remain absent; no Passport authority moves into Omnigate.
...
//! RO:TEST — `cargo test -p omnigate --test crabnode_cn4_register_root_proof_proxy`.
...
async fn dummy_passport() -> SocketAddr {
    async fn handler(headers: HeaderMap, body: Bytes) -> (StatusCode, Json<Value>) {
        ...
        (
            StatusCode::OK,
            Json(json!({
                "path": "/v1/passport/register/proof",
                "body": body,
    ...
    serve(Router::new().route("/v1/passport/register/proof", post(handler))).await
}
...
#[tokio::test]
async fn fixed_register_root_proof_route_proxies_exactly() {
    ...
    let response = reqwest::Client::new()
        .post(format!(
            "http://{omnigate}/v1/identity/passport/register/proof"
        ))
        .header("authorization", "Bearer cn4")
        .header("x-correlation-id", "corr-cn4-proof")
        .header("x-request-id", "req-cn4-proof")
    ...
    assert_eq!(response["path"], "/v1/passport/register/proof");
    assert_eq!(response["body"], body);
    assert_eq!(response["authorization"], "Bearer cn4");
    assert_eq!(response["correlation"], "corr-cn4-proof");
    assert_eq!(response["request_id"], "req-cn4-proof");
    assert!(
...
#[tokio::test]
async fn fixed_register_root_proof_route_enforces_16_kib_cap() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
...
#[tokio::test]
async fn nearby_unreviewed_prove_remains_unmounted() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
    ...
    let response = reqwest::Client::new()
        .post(format!(
            "http://{omnigate}/v1/identity/passport/prove/extra"
        ))
