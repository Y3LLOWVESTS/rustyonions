...
//! RO:WHAT — Tooling boundary tests for omnigate QuickChain Phase-0.
...
//! RO:TEST — `cargo test -p omnigate --test quickchain_tooling_boundary`.
...
fn preflight_script_discovers_quickchain_tests_dynamically_and_runs_hard_gates() {
    ...
    for required in [
        ...
        "== omnigate quickchain exhaustive preflight gate passed: tests=",
    ] {
        assert!(
...
fn no_python_helper_drift_exists_under_crate() {
    ...
    assert!(
        offenders.is_empty(),
        "omnigate QuickChain tooling must remain Bash/cargo-only; unexpected Python files:\n{}",
        offenders
            .iter()
