//! RO:WHAT — CN-4 acceptance for Omnigate's fixed ProveSession challenge/proof proxy pair.
//! RO:WHY — Device possession must traverse the reviewed 9090 façade to svc-passport without moving identity authority into Omnigate.
//! RO:INTERACTS — Omnigate v1 router, Passport proxy, dummy loopback svc-passport.
//! RO:INVARIANTS — exact fixed downstream paths; opaque body/header forwarding; 16 KiB cap; no capability or username mutation.
//! RO:METRICS — none.
//! RO:CONFIG — process-local OMNIGATE_PASSPORT_BASE_URL override.
//! RO:SECURITY — no RecoveryRoot, root PIN, device private key, capability issuance, wallet, or ledger authority.
//! RO:TEST — cargo test -p omnigate --test crabnode_cn4_device_session_proxy.
...
async fn dummy_passport() -> SocketAddr {
    ...
    serve(
        Router::new()
            .route("/v1/passport/challenge", post(handler))
            .route("/v1/passport/prove", post(handler)),
    )
    .await
}
...
#[tokio::test]
async fn fixed_device_session_pair_proxies_exactly() {
    ...
    for (public_path, downstream_path, marker) in [
        ...
        ("/v1/identity/passport/prove", "/v1/passport/prove", "prove"),
    ] {
        let payload = json!({
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await
            .expect("ProveSession proxy response");
...
#[tokio::test]
async fn fixed_device_session_pair_enforces_16_kib_cap() {
    let _guard = ENV_LOCK.lock().await;
    clear_env();
