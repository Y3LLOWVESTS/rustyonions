...
const DEFAULT_PUBLICATION_LIMIT: usize =
    12;

const MAX_PUBLICATION_LIMIT: usize =
    24;

const DEFAULT_CREATOR_LIMIT: usize =
    12;

const MAX_CREATOR_LIMIT: usize =
    24;

const DEFAULT_SITE_LIMIT: usize =
    8;

const MAX_SITE_LIMIT: usize =
    16;
    ...
    Lazy::new(
        || {
            reqwest::Client::builder()
                ...
                .build()
                .expect(
                    "omnigate Explore client should build",
                )
        },
    );
...
pub async fn get_explore_discovery(
    Query(query):
        Query<ExploreDiscoveryQuery>,
) -> Response {
    let publication_limit =
        match normalize_limit(
...
fn bad_request(
    ...
    let message =
        match field {
            "publicationLimit" =>
                "publicationLimit is outside the reviewed Explore bound",

            "creatorLimit" =>
                "creatorLimit is outside the reviewed Explore bound",

            _ =>
                "siteLimit is outside the reviewed Explore bound",
        };
    ...
    (
        ...
        Json(
            UpstreamProblem {
                ...
                message:
                    "svc-index Explore discovery is unavailable",
