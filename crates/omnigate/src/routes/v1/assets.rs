...
const VIDEO_RENDITION_GROUP_HEADER_LIMIT_BYTES: usize = 12 * 1024;
...
fn build_video_manifest(
    ...
    root.insert(
        "rendition".to_owned(),
        json!({
            "role": grab(headers, "x-ron-video-rendition-role"),
            "label": grab(headers, "x-ron-video-rendition-label"),
            "asset_kind": "video",
            "cid": asset_cid,
            "crab_url": format!("crab://{}.video", asset_cid.trim_start_matches("b3:")),
            "mime": grab(headers, "content-type"),
        }),
    );

    if let Some(rendition_group) = video_rendition_group_from_headers(headers, asset_cid) {
        root.insert("rendition_group".to_owned(), rendition_group);
    }
...
fn image_rendition_group_from_headers(headers: &HeaderMap, asset_cid: &str) -> Option<Value> {
    ...
    if !rendition_group_contains_asset(&value, asset_cid) {
        return None;
    }
...
fn video_rendition_group_from_headers(headers: &HeaderMap, asset_cid: &str) -> Option<Value> {
    let raw = grab(headers, "x-ron-video-rendition-group")?;

    if raw.len() > VIDEO_RENDITION_GROUP_HEADER_LIMIT_BYTES {
        return None;
    }
...
fn rendition_group_contains_asset(value: &Value, asset_cid: &str) -> bool {
    let Some(renditions) = value.get("renditions").and_then(Value::as_array) else {
        return false;
    };

    renditions.iter().any(|item| {
        value_string(item, "cid")
            .or_else(|| value_string(item, "asset_cid"))
            .or_else(|| value_string(item, "assetCid"))
            .or_else(|| value_string(item, "content_id"))
            .or_else(|| value_string(item, "contentId"))
            .or_else(|| value_string(item, "predicted_cid"))
            .or_else(|| value_string(item, "predictedCid"))
            .as_deref()
            == Some(asset_cid)
    })
}
