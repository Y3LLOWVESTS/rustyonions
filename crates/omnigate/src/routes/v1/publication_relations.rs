...
static HTTP_CLIENT: Lazy<reqwest::Client> = Lazy::new(|| {
    reqwest::Client::builder()
        ...
        .use_rustls_tls()
        .build()
        .expect("omnigate publication relation client should build")
});

#[derive(Debug, Clone, Deserialize, PartialEq, Eq)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
pub struct PublicationRelationQuery {
    pub parent_crab_url: String,
...
pub async fn list_publication_relations(
    Query(query): Query<PublicationRelationQuery>,
...
async fn proxy_to_index(query: PublicationRelationQuery, headers: HeaderMap) -> Response {
    let raw_url = format!(
...

    {
        ...
        pairs.append_pair("parentCrabUrl", &query.parent_crab_url);
