...
**Future:**  
* [ ] Schema-generated DTOs from `ron-proto` (see §11)  
...
**RO:WHAT —**
Ensure all SDKs (Rust, TS, Go, etc.) share a single structural truth for DTOs, errors, pagination, and facets so that **same capability + same request ⇒ same behavior**, regardless of language.

**RO:INVARIANTS —**

* All DTOs exposed by SDKs are derived from canonical schemas (`ron-proto` + HTTP/OpenAPI).
* No SDK invents conflicting request/response shapes by hand.
* Facet manifests (TOML) follow a unified schema profile (see `SDK_SCHEMA_IDB.MD`), even when generated via SDK builders.
* Error and pagination envelopes have the same fields and semantics across languages.

#### 11.1 Canonical Sources

* `ron-proto` — message definitions for OAP/1, storage/index DTOs, auth/passport payloads, audit/registry entries (where exposed to SDKs).
* HTTP/OpenAPI spec for gateway/omnigate — path/method/body and error envelopes.
* `SDK_SCHEMA_IDB.MD` — cross-language type mapping (Rust/TS/Go/etc.), JSON encoding rules, and facet manifest schema.

#### 11.2 SDK Requirements

**Tier 1 SDKs (Rust, TS, Go, etc.):**

* MUST use codegen or direct bindings from `ron-proto`/OpenAPI for DTOs and error envelopes wherever tooling allows.
* MUST implement the canonical error shape and pagination envelope defined in `SDK_SCHEMA_IDB.MD`.
* MUST treat facet manifests (TOML) as **data**, not behavior; any code that generates TOML must comply with the facet schema (kind, routes, paths, security hints, limits, etc.).

**Tier 2 SDKs:**

* SHOULD use codegen when possible; otherwise:

  * MUST match the same JSON field names and types as canonical schemas.
  * MUST pass the shared interop tests against a local node.

**Tier 3 SDKs:**

* MAY handwrite DTOs but:

  * MUST document mapping against canonical schema.
  * SHOULD run at least a minimal interop test suite.

#### 11.3 Type Mapping & Drift Avoidance

* Document any tricky mappings (e.g., `u64` → string/bigint in TS, timestamps as ISO8601 strings) and ensure they are consistent with `SDK_SCHEMA_IDB.MD`.
* Lossless representation is required where practical; where not, behavior must be documented and tested.
* CI MUST include a schema drift check:

  * For languages with codegen: regenerate DTOs from schema and fail build if git diff is non-empty.
  * For all SDKs: run a shared “interop tests” suite that hits a local node (micronode/macronode) and asserts DTO, error, and pagination shapes.

#### 11.4 Facet Interop

* All SDKs treat facets as **declarative route definitions via TOML manifests**, not embedded code.

* Actual HTTP path semantics are fixed and documented:
...
* SDKs that provide facet helpers/builders MUST:

  * Generate manifests that validate under Micronode’s facet loader.
  * Respect the unified facet schema (kinds: `static`, `echo`, future `proxy`; security hints; limits; integrity options).
  * Clearly document how language runtime code (Node/TS, Python, etc.) interacts with these facets (e.g., proxy facets forwarding to a backend service).

* For proxy facets (future):

  * SDKs may provide builders to configure upstream (host/port/path) and security/token/cert settings.
  * Upstream auth and caps MUST be explicit configuration (no silent, insecure defaults).
