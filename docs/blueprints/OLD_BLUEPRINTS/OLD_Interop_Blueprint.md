...
### 3.1.1 Object Addressing (Normative)
All content-addressed objects use **BLAKE3-256**, encoded as `b3:<hex>` (lowercase). Clients MAY send truncated prefixes for routing;
services MUST verify full digest equality before returning bytes. Future multihash/CID variants MUST be prefixed distinctly.
...
- SDKs (TS/Python/Swift) MUST implement retries with exponential backoff (jitter), default max **3 attempts**; map **429→RetryAfterError** (respect `Retry-After`), **503→TemporaryUnavailable**, and frame/codec errors→**ProtocolError**. Provide one end-to-end example per SDK.
...
Chunks 64KiB; Merkle manifests for multi‑chunk; Address `b3:<hex>`.  
...
[index: {offset:u64, len:u32, b3:32, name_len:u16, name:bytes} * count]
